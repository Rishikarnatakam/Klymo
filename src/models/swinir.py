...
Provides a clean interface to the pretrained SwinIR model
for 4× super-resolution of satellite imagery.
"""
...
# ============================================================================
# SwinIR Architecture Components
# ============================================================================
...
class Upsample(nn.Sequential):
    """Upsample module for image super-resolution."""
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.upsample = nn.Sequential(nn.Conv2d(embed_dim, (upscale ** 2) * num_out_ch, 3, 1, 1),
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            x = self.conv_first(x)
            x = self.conv_before_upsample(self.forward_features(x) + x)
            x = self.conv_last(self.upsample(x))
        elif self.upsampler == 'pixelshuffledirect':
            x = self.conv_first(x)
            x = self.upsample(self.forward_features(x) + x)
        elif self.upsampler == 'nearest+conv':
            x = self.conv_first(x)
            x = self.conv_before_upsample(self.forward_features(x) + x)
...
def load_swinir_model(
    ...
    if model_path is None:
        model_path = MODEL_DIR / SWINIR_MODEL_NAME
        
        if not model_path.exists():
            success = download_model(SWINIR_MODEL_URL, model_path)
            if not success:
                raise RuntimeError("Failed to download SwinIR model")
    ...
    if model_path.exists():
        ...
        # Filter out attn_mask buffers as they are runtime-computed (not learned)
        # and may have different sizes depending on input image size
        model_dict = model.state_dict()
        filtered_dict = {}
        
        for k, v in pretrained_dict.items():
            if 'attn_mask' in k:
                # Skip attention mask buffers - they get recomputed at runtime
                continue
            if k in model_dict:
...
class SwinIRWrapper:
    """
    Wrapper class for SwinIR model with convenient interface.
    ...
    def __init__(
        ...
        self.name = f"SwinIR {scale}×"
        
        # Load model
        print(f"Initializing SwinIR on {self.device}...")
        self.model = load_swinir_model(
    ...
    def __call__(
        ...
        """Alias for inference method."""
        return self.inference(image)
    
    def to(self, device: str) -> "SwinIRWrapper":
        """Move model to specified device."""
        self.device = device
        self.model = self.model.to(device)
        return self
    
    def eval(self) -> "SwinIRWrapper":
